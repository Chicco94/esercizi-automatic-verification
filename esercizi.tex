...
% per rimuovere inutile spazio bianco dopo gli align
\usepackage{etoolbox}
...
% per elenchi puntati più gestibili in termini di spazio
% https://ctan.mirror.garr.it/mirrors/ctan/macros/latex/contrib/enumitem/enumitem.pdf
\usepackage{enumitem}
...
%    \path[->] 
%    (q_0) edge  node {$\alpha(0,k_0)$} (q_1)
%    (q_1) edge  node {$\alpha(k_0,k)$} (q_2)
%    (q_2) edge  node {$\alpha(k,m)$} (q_3)
%    (q_3) edge  node {$w$} (q_4)
%    (q_3) edge  node {$w'$} (q_5);
%   
%   % annotazioni
%	\draw[decoration={brace,mirror,raise=5pt},decorate] 
%   		(q_1.south) -- node[below=6pt] {$\alpha(k_0,m), x \in W_V$} (q_3.south);
%	\draw[decoration={brace,raise=5pt},decorate] 
%   		(q_1.north) -- node[above=6pt] {$v\in V$} (q_2.north);
%	\draw[decoration={brace,raise=5pt},decorate] 
%   		(q_2.north) -- node[above=6pt] {$u\in U$} (q_3.north);
%\end{tikzpicture}
%\end{center}
%
%\begin{center}
...
%   \node[state] (q_5) [above right=of q_4] {};
%   \node[state] (q_6) [below right=of q_4] {};
%    \path[->] 
%    (q_0) edge  node {$\alpha(0,k_0)$} (q_1)
%    (q_1) edge  node {$\alpha(k_0,k)$} (q_2)
%    (q_2) edge  node {$\alpha(k,m)$} (q_3)
%    (q_3) edge  node {$\alpha(m,m')$} (q_4)
%    (q_4) edge  node {$w$} (q_5)
%    (q_4) edge  node {$w'$} (q_6);
%   
%   % annotazioni
%	\draw[decoration={brace,mirror,raise=5pt},decorate] 
%   		(q_1.south) -- node[below=6pt] {$\alpha(k_0,m), x \in W_V$} (q_3.south);
%   \draw[decoration={brace,mirror,raise=30pt},decorate] 
%   		(q_1.south) -- node[below=31pt] {$\alpha(k_0,m'), x' \notin W_V$} (q_4.south);
%	\draw[decoration={brace,raise=5pt},decorate] 
%   		(q_1.north) -- node[above=6pt] {$v\in V$} (q_2.north);
%   \draw[decoration={brace,raise=5pt},decorate] 
%   		(q_2.north) -- node[above=6pt] {$u\in U$} (q_3.north);
%   \draw[decoration={brace,raise=30pt},decorate] 
%   		(q_2.north) -- node[above=31pt] {$\alpha(k,m'), u' \in U'$} (q_4.north);
%\end{tikzpicture}
%\end{center}
...
Un linguaggio star-free è costruito su:
\begin{enumerate}[noitemsep]
\item un alfabeto $A$
\item l'insieme vuoto $\emptyset$
\item operatori booleani \emph{AND} \emph{OR} \emph{NOT}
\item operazione di concatenazione $\cdot$
\end{enumerate}
Mentre un linguaggio del frammento al prim'ordine di $S1S_A$ è costruito su:
\begin{enumerate}[noitemsep]
\item un alfabeto $A$
\item una costante $0$
\item operatori booleani \emph{AND} \emph{OR} \emph{NOT}
\item quantificatore $\forall$ e $\exists$
\item un insieme di variabili $\{x,y,\ldots\}$
\item un insieme di predicati $\{<\} \cup \{Q_a \forall a \in A\}$
\end{enumerate}

L'idea è quella di ricostruire l'espressione regolare che definisce il linguaggio L come una formula in $S1S_A$.
Di seguito elenco delle forme di $S1S_A$ con cui vengono interpretate le espressioni regolari costruite su $L$:
\begin{itemize}
	\item $\phi_\epsilon(x) = \bigwedge_{a \in A} x \notin Q_a$
	\item $\phi_a(x) = x \in Q_a \bigwedge_{b \in A,b \neq a} x \notin Q_b$
	\item $\phi_{exp_1 \cap \exp_2}(x) = \phi_{exp_1}(x) \land \phi_{exp_2}(x)$
	\item $\phi_{exp_1 \cup \exp_2}(x) = \phi_{exp_1}(x) \lor \phi_{exp_2}(x)$
	\item $\phi_{\neg exp_1}(x) = \neg \phi_{exp_1}(x)$
	\item $\phi_{exp_1 \cdot \exp_2}(x,y) = \phi_{exp_1}(x) \land x+1 = y \land \phi_{exp_2}(y)$
\end{itemize}
Con queste formule di $S1S_A$ possiamo quindi ricostruire l'espressione regolare che caratterizza $L$. 
...
\begin{tikzpicture}[shorten >=1pt,node distance=2.5cm,on grid,auto] 
   ...
   \node[state,accepting] (q_2) [below right=of q_0] {$q_2$};
    ...
    (q_0) edge  node {$a$} (q_1)
    (q_0) edge  node {$b$} (q_2)
    (q_1) edge [loop below] node {$a$} (q_1)
