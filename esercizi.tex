...
\textit{Consider a program P of the form $$P :: \left[declaration; \left[[ l_1:S_1;\hat{l_1}:] \parallel \ldots \parallel [l_k:S_k;\hat{l_k}:]\right]\right]$$}
...
\begin{enumerate}
\item $[S_1 \textbf{or} S_2] \approx [S_2 \textbf{or} S_1]$
\item $[S_1 \parallel S_2] \approx [S_2 \parallel S_1]$
\item $S \approx [S; \textbf{skip}]$
\item $\textbf{await}\;c \approx \textbf{while}\;\neg c\;\textbf{do skip}$
\end{enumerate}}

\medskip
\textit{\textbf{d)} Are the two statements:
$$ \textbf{await} x \;\;\; and \textbf{skip};\; m: \textbf{await} x$$
