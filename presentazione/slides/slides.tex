...
{\cite[On the Synthesis of Discrete Controllers for Timed Systems]{first_article}}
...
\begin{frame}
	\begin{dfn}[Controlled Trajectories]
		...
		\begin{itemize}
			...
			\item a $t-step$ for some $t \in T$ such that for every $t'$, $t' \in [0,t), C(q,x+\textbf{1}t')=\perp$
		\end{itemize}
	\end{dfn}
...
\begin{frame}
	...
	Then we define a function $\delta : (Q \times  X) \times (T \times \Sigma^\perp_c) \mapsto 2^{Q \times X}$ where $\delta(\conf,(t,\sigma))$ stands for all the possible consequences of the controller attempting to issue the command $\sigma \in \Sigma^\perp_c$ after waiting $t$ time units starting at configuration $\conf$
...
\begin{frame}
	...
	\note{
		...
		$t$ will lead to a winning position. Hence by letting $C\conf = \perp$ when $t_1>0$ and
