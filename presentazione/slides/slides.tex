...
\begin{frame}
	\begin{dfn}[Real time Controller Synthesis]
		Given a timed automaton $\mathcal{T}$ an a acceptance condition $\Omega$, the problem \textbf{RT-Synth}$(\mathcal{T},\Omega)$ is: Construct a real-time controller $C$ such that $L_C(\mathcal{T}\subseteqL(\mathcal{T},\Omega))$
	\end{dfn}
\end{frame}
\end{document}
